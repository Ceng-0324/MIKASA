...
微信第一阶段增加 Hermes 原生扫码入口、本机凭据绑定与不联网诊断。专项 23 项、全量 191 项通过（255.972 秒），文档、doctor 和 diff 检查通过。覆盖缺少扫码用户标识时拒绝覆盖、0600 凭据、错误脱敏、临时登录 home 隔离与飞书 profile 保留。真实 iLink 已成功返回登录二维码，尚未完成扫码或消息验收。微信消息将直接复用 Hermes 多平台 Gateway，不新增消息路由器。
