...
日期：2026-09-20。对象：基线 `8aee0de` 加本轮执行进度持久化变更，Python 3.12.13。本记录不证明后续变更。

## 已执行

| 检查 | 结果 |
| --- | --- |
| `python3.12 -m unittest discover -q` | 87 项通过，51.600 秒 |
...
| 真实认证值字节扫描 | 1051 个 runtime/state 文件、88 个 Git 可纳入文件未发现匹配；本地配置及联调报告权限 0600 |
| Hermes/CCH 执行进度探针 | 实现红绿修复、宿主验证/提交和只读拆解均通过；事件实时持久化，84.779 / 36.705 秒，见 [工具覆盖验证](HERMES_TOOLS_VALIDATION.md) |
| 人格与工程 skill | 本轮真实任务加载 persona/implement 与 persona/plan，宿主指纹核对通过；其他路由由本地 doctor 与隔离回归验证 |

历史分页和无工具人格验证见 [工具覆盖记录](HERMES_TOOLS_VALIDATION.md)，本轮未重复这两个真实场景。历史验证另保留：[首次 Hermes/CCH 联调](HERMES_CCH_VALIDATION.md)、[聊天模型切换](CHAT_VALIDATION.md)。本轮没有重新运行真实 HTTP 聊天切换，聊天相关隔离回归包含在上述 87 项中。
...
本轮新增：运行中进度可查询、模型失败/超时后的证据持久化、取消后旧进度拒绝写入、重试令牌隔离、runner 恢复保留阶段、写入失败先于工具副作用中止、验证和提交阶段记录。
