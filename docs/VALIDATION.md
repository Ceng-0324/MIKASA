...
本轮精简：移除旧 `feishu` 别名，消息统一走 `gateway --platform`；专项 27 项、全量 195 项 unittest 通过（257.557 秒），文档、默认及本机配置 doctor、CLI 帮助和 diff 检查通过。覆盖账号绑定、凭据隔离、原生启动、消息准入、任务/调度、回执、备份恢复及失败路径。
...
本文件只维护当前结论和限制，逐轮日志保留在 Git 历史。清理前详细记录可用 `git show 18563e6:docs/VALIDATION.md` 查看；更早报告见 `git show 3b436b1:docs/VALIDATION.md`。旧验证不证明后续代码；每轮实际检查结果随提交更新。

运行状态、会话、记忆、凭据和请求回执留在本机。过期二维码、已关闭的调试日志和可再生成字节码可以清理；固定 Hermes 源码及 Python 环境虽位于 `runtime/cache`，仍是运行依赖。

本轮清除 3 张旧二维码、5 份停用日志、旧飞书配置及临时扫码工具；删除 16,459 个字节码文件（约 95 MiB）。保留正在写入的日志、固定依赖与正式状态，不重启 Gateway。
