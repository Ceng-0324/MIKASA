...
原生工具状态、阶段说明、长任务通知和最终结果回到原聊天。默认 `tool_progress: all`，逐工具播报，保留模型中途说明和 15 秒长任务心跳；Hermes 的投递层仍可按平台合并同类进度。微信不支持编辑时，Hermes 仍发送逐工具阶段、心跳及完整结果。后台原始通知采用 `error`，保留失败提示和原生 Agent 唤醒，成功结果由当前任务汇报；交付前读取后台进程的 wait/log 结果，避免遗漏和重复通知。阶段说明取决于模型实际生成，不保证固定条数。
