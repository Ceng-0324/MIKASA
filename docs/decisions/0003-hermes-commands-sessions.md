...
`/new` 与 `/reset` 在同一事务创建新 ID、保留所选模型、记录事件和旧会话命令回执；新会话 revision=0、无历史。旧记录保留且仍按原账号鉴权。原请求幂等键重放只返回同一个新 ID；暂停或事务失败不创建半个会话。HTTP 回复的 `chat_id` 是下一条消息应使用的 ID，网页和 CLI 自动跟随。手动恢复旧会话仍可继续它，因此新旧会话均保持独立有效。
