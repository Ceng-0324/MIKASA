...
[0008：工程持续会话与账号记忆](0008-engineering-state.md) 将修复和重试接入原生 SessionDB，工程与聊天共用提交账号的原生记忆文件，保留任务执行隔离和旧数据档案。

[0009：修复循环交回 Hermes](0009-native-repair-loop.md) 删除宿主按固定次数重启 worker 的循环，保留最终独立验收、失败现场与显式重试证据；明确 Kanban 后续迁移的接入边界。
