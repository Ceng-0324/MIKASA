...
def main():
    ...
    checks = {}
    with tempfile.TemporaryDirectory(prefix='mkeng-', dir='/tmp') as directory:
        ...
        prompt.write_text(f'''这是隔离的原生工程验收，请在当前合成仓库完成并验证：
...
不推送、不发布、不访问外部账号、不修改仓库外文件（原生记忆除外）。最后自然语言说明实际结果，不输出 JSON。''')
        ...
        checks['native_cli_exit'] = result.returncode == 0
        ...
        report_loading = [{k: e.get(k) for k in ('session_id','identity','policy','persona_skill','skills_index')}
                          for e in requests]
        ...
        report = {'hermes_revision': HERMES_REVISION, 'requested_model': args.model,
                  'checks': checks, 'tools_observed': tools, 'request_loading': report_loading,
                  'reported_models': sorted({e['reported_model'] for e in evidence if e.get('reported_model')}),
                  'passed': all(checks.values()),
                  'scope': 'disposable native CLI repo, no platform writes; scheduler help is not a dispatched task'}
