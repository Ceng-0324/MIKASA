...
运行时将当前文件内容按 manifest 加载至 Hermes system message，并记录 SHA-256 与来源。它不依赖 Hermes 自带的 skills 浏览工具，也不会把项目 skill 安装到用户全局目录。原上游使用的交互问答、其他 skill 调用、并行 agent 与发布工具在当前受限工具与 JSON worker 中不可直接执行；这些差异明确保留在上表，不能把本地适配宣称为上游完整工作流。
