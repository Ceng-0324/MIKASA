...
聊天只自动加载人格，实质工程讨论再读取原生 `mikasa-engineering` 及对应方法 skill。`mikasa-engineering` 在初始化时从两个 canonical 工程文档生成到 profile，不在仓库维护副本。方法 skills 服务于讨论与执行；JSON 字段、任务依赖索引、快照导入和发布责任由工程 Worker.request 的调用协议承载，日常聊天不套 worker 输出格式。
