...
`worker.max_attempts` 已停用：宿主不再按次数重启 Agent 修复任务，检查与修复由 worker 内部工具循环完成。v1 配置暂接受旧整数值并由 doctor 的 `deprecated_settings` 提示；示例已删除此字段，迁移时可从本机配置移除。兼容仅用于已有配置不因升级无法启动；退出条件是下一版配置迁移同步删除旧字段。Hermes 工程仍受单次 24 次迭代、128 个工具证据事件、worker.timeout 与进程输出上限约束，详见 [0009](../docs/decisions/0009-native-repair-loop.md)。
