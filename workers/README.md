...
当前执行器为 [Hermes JSON 桥接](hermes/README.md)；Codex、Claude 保留为可选接入，不是当前运行依赖。协议由 `mikasa/worker.py` 定义：单次 stdin JSON 请求、stdout JSON 结果，输出字段按任务类型校验，错误输出不持久化，以免泄露模型凭据。
