...
class HermesCommandTests(BaseTest):
    ...
    def test_command_rpc_does_not_read_or_inherit_credentials(self):
        ...
        with patch.dict(os.environ, {"MIKASA_MODEL_API_KEY": "private", "GH_TOKEN": "private"}), \
                patch("mikasa.model_settings.model_environment", side_effect=AssertionError("must not read credentials")), \
                patch("mikasa.commands.run", return_value={"code": 0, "stdout": json.dumps(envelope)}) as run:
            self.assertEqual(resolve(self.config, "/model", lambda: False)["kind"], "status")
        env = run.call_args.kwargs["env"]
        self.assertNotIn("MIKASA_MODEL_API_KEY", env)
        self.assertNotIn("GH_TOKEN", env)
        self.assertEqual(json.loads(run.call_args.kwargs["stdin"]), {"text": "/model"})
    ...
    def test_bad_command_rpc_fails_closed_without_sdk_details(self):
        for code, output in [(1, "private SDK diagnostic"), (0, '{"version":1,"result":{}}'),
            ...
            with patch("mikasa.commands.run", return_value={"code": code, "stdout": output}), \
                    self.assertRaisesRegex(MikasaError, "命令未执行"):
                resolve(self.config, "/model", lambda: False)
