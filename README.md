...
历史真实 CCH 联调已验证合成工程任务和 GPT/Claude 会话切换；`default` 分组仍缺网关侧确认。GitHub 只读接入探针与 Hermes 原生飞书负责人单聊入口已实现，真实平台权限、消息往返和目标 VM 尚未验收。账号、凭据、飞书应用与用户 ID 的获取步骤见[接入手册](docs/runbooks/CONNECTIONS.md)。后续顺序为 **GitHub/飞书 → VM → 聊天工程任务与 FluxCore 联合验收**，详见[推进计划](MIKASA_FUNCTION_PLAN.md)与[验证边界](docs/VALIDATION.md)。
