...
更新日期：2026-09-20。本文区分已确认方向、本地实现和外部验收。人格与工程规则 1.0 已形成，Mikasa 运行时 0.1 已实现下述核心工作流，并通过 46 项隔离测试；真实模型、账号权限和 VM 运行尚未验收。证据见 [验证记录](docs/VALIDATION.md)。
...
| 模型执行 | 固定版本 Hermes SDK 的 JSON 桥接、canonical 注入、工具禁用、专用 home、模型环境变量白名单 |
...
- **工程 skills**：以 [mattpocock/skills](https://github.com/mattpocock/skills) 为候选来源，实际条目、版本、许可、触发方式、安装位置及适配验证待核实；当前未安装。
- **模型执行**：已实现 Hermes 桥接；CCH 兼容端点通过环境变量注入，真实模型名、端点、凭据和 Hermes 安装待提供并验证。Codex/Claude 不是当前必需执行器，仍保留为后续可选项。
