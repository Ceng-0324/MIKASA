...
当前优先执行完整原生工程迁移：`engineer --cwd 仓库 -- chat` 直接使用 Hermes CLI，保留完整 Git 工作区、原生工具、skills、插件、预算和会话。工程 profile 独立管理会话，与同账号聊天共享长期记忆。正在迁移旧 worker/快照/串行 runner 的调用方；完成退休、备份和 VM 合成工程验证后，再推进聊天工程入口与 FluxCore。
